...
class LoadCVBundleEntityExperience extends Fixture implements OrderedFixtureInterface, ContainerAwareInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $item1->setDateDebut(new \DateTime("2017-06-19 00:00:00"));
        $item1->setDateFin(new \DateTime("2017-09-15 00:00:00"));
        $item1->setVille("Mérignac");
        $item1->setIntitule("Stagiaire Développeur web");
        $item1->setEntreprise("SPIE ICS");
        $item1->setType("Stage");
        $item1->setDescription("Conception d'une plateforme d'e-commerce avec une partie client, et une partie administration.
Fonctionnalités :
...
- Gestion des entités de la plateforme depuis la partie Administration selon des rôles et des droits");
        $item1->setOrdre(1);
        $item1->setCodepostal(33);

        $manager->persist($item1);

        $item2 = new Experience();
        $this->addReference('_reference_CVBundleEntityExperience2', $item2);
        $item2->setDateDebut(new \DateTime("2016-07-01 00:00:00"));
        $item2->setDateFin(new \DateTime("2016-08-30 00:00:00"));
        $item2->setVille("Royan");
        $item2->setIntitule("Equipier Formation Cuisine");
        $item2->setEntreprise("Mc Donald's");
        $item2->setType("Job d'été");
        $item2->setDescription("- Préparation des sandwichs, Cuisson des produits
